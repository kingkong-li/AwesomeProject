...
function reportToCustomException(e: ExtendedError, isFatal: boolean) {
  console.log("reportToCustomException")
  const {CustomExceptionsModule} = require('NativeModules');
  if (CustomExceptionsModule) {
    const parseErrorStack = require('parseErrorStack');
    const stack = parseErrorStack(e);
    const currentExceptionID = ++exceptionID;
    const message =
      e.jsEngine == null ? e.message : `${e.message}, js engine: ${e.jsEngine}`;
    if (isFatal) {
      CustomExceptionsModule.reportFatalException(
        message,
        stack,
        currentExceptionID,
      );
    } else {
      CustomExceptionsModule.reportSoftException(message, stack, currentExceptionID);
    }
    if (__DEV__) {
      const symbolicateStackTrace = require('symbolicateStackTrace');
      symbolicateStackTrace(stack)
        .then(prettyStack => {
          if (prettyStack) {
            CustomExceptionsModule.updateExceptionMessage(
              e.message,
              prettyStack,
              currentExceptionID,
            );
...
function handleException(e: Error, isFatal: boolean) {
  ...
  if(Platform.OS==="android"){
    reportToCustomException(e, isFatal);
  }else{
